...
import { getDataWaterProduction } from '@/services/badiklat/dashboard';
...
export const ProduksiAir: FunctionComponent<ViewLogTypes> = ({
  loading,
  // air_bersih,
  // air_lumpur,
  // air_beku,
}) => {
  const { isFetching, data } = useQuery(['waterproduction'], {
    queryFn: async () => {
      const d = await getDataWaterProduction({});
      if (!d.error) {
        return d.data;
      } else {
        return {};
