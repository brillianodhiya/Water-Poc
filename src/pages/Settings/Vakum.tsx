...
import {
  ApiEditWaterUsageAnomali,
  ApiEditWaterVacuumParam,
  getWaterPressureVacuum,
...
export const CardVakum: FunctionComponent<ViewLogTypes> = ({ settings, getDataInit }) => {
  ...
  const getData = async () => {
    setLoading(true);
    const data = await getWaterPressureVacuum({});
  ...
  const onFinish = async (values: any) => {
    ...
    setIsEdit(false);
    setLoading(true);
    const data = await ApiEditWaterVacuumParam(values.device);
