...
import {
  ApiEditWaterPressureSetting,
  getWaterPressureSettings,
...
export const CardPressure: FunctionComponent<ViewLogTypes> = ({ settings, getDataInit }) => {
  ...
  const getData = async () => {
    setLoading(true);
    const data = await getWaterPressureSettings({});
  ...
  const onFinish = async (values: any) => {
    ...
    setIsEdit(false);
    setLoading(true);
    const data = await ApiEditWaterPressureSetting(values.device);
