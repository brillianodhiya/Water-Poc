...
export async function getDevice(options: object | undefined) {
  ...
  try {
    const { data } = await api.get<APINebula.DefaultApiObj>('/device', {
      method: 'GET',
...
export async function getTicketNodeType(options: object | undefined) {
  // console.log(token, 'TOKEN');

  try {
    const { data } = await api.get<APINebula.DefaultApiObj>('/ticket/type/node/list', {
      method: 'GET',
...
export async function getTechnician(options: object | undefined) {
  ...
  try {
    const { data } = await api.get<APINebula.DefaultApiObj>('/user/bita/list?role=teknisi', {
      method: 'GET',
