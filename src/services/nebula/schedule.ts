...
export const ApiSetSchedule = async (values: {
  ...
  try {
    const { data } = await api.post<APINebula.PostResponse>('/schedule/create', {
      room_id: values.id,
...
export const ApiEditSchedule = async (values: {
  ...
  try {
    const { data } = await api.patch<APINebula.PostResponse>('/schedule/edit', {
      room_id: values.id,
      room_type: values.type,
      room_name: values.name,
      scedule_type: values.scedule_type,
...
export async function apiGetListEvent(values: {
  ...
  try {
    const { data } = await api.post<APINebula.ApiListEvent>(
...
export async function apiGetListEventDay(values: {
  ...
  try {
    const { data } = await api.post<APINebula.ApiListEvent>('/schedule/view/' + values.date, {
      room_type: values.room_type,
      room_id: values.room_id,
    });
...
export const ApiDeleteSchedule = async (values: { id?: number }) => {
  try {
    const { data } = await api.delete<APINebula.PostResponse>('/schedule/' + values.id);
